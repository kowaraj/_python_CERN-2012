...
def test_timer_should_report_approximately_correct_times(seconds_to_sleep):    
    with timer() as t:
        ...
        pass
    print t.__dict__
    assert abs(t.time - seconds_to_sleep) < 0.001

test_timer_should_report_approximately_correct_times(2)    
...
def xtest_stream_redirect_should_be_able_to_unpack_into_3_separate_values():
...
def xtest_stream_redirect_attribute_names_should_be_out_err_in_():
...
def xtest_nested_stream_redirection_should_work():
...
def xtest_stream_redirection_should_reset_streams_when_exception_raised():
...
def xtest_should_be_able_to_redirect_just_one_stream_by_keyword_argument():
